// lib/silkdb.js — Shared Atlas + tie-break engine for Compare and Trade.
import pkg from 'mongodb';
const { MongoClient } = pkg;

let client, collection, tradeCollection;
...
export async function getValueCollection() {
  ...
  collection = c.db('silk_bot').collection('value_list');
  return collection;
}

export async function getTradeCollection() {
  if (tradeCollection) return tradeCollection;
  const c = await ensureClient();
  tradeCollection = c.db('silk_bot').collection('trade_ads');
  return tradeCollection;
}

// --- Faithful port of Python difflib.SequenceMatcher.ratio ---
...
// --- Per-item lookup (tightened, word-boundary level detection) ---
export async function fetchCompareItem(rawQuery) {
  ...
  try {
    ...
    const isLvl10 = /\b(10|max|lvl\s*10|level\s*10)\b/i.test(lower);
